...
/*
    Given
        - 테스트 유저를 생성, jjwt 라이블러리 이용해서 리프레시 토큰을 만들어 DB에 저장,
        토큰 생성 API의 요청 본문에 토큰을 포함하여 요청 객체를 생성
    ...
    Then
        - 응답 코드가 201 Created인지 확인하고, 응답으로 온 섹세스 토큰이
        비어있지 않은지 확인
 */
@SpringBootTest
    ...
    chapter09
        ...
        JWT : 토큰 기반 인증 중 현재 인기 있는 방식 JSON 형식으로
            ...
            구성 :
                ...
                헤더 - 토큰의 타입과 해싱 알고리즘(JWT / HS245을 적용)
                내용 - 토큰에 담을 정보
                서명 - 토큰이 조작되었거나 변경되지 않았음을 확인하는 용도
            Refresh Token : 엑세스 토큰과 별개의 토큰. 엑세스 토큰이 만료되었을 때
                새로운 엑세스 토큰을 발급받는 용도
                (CreateTokenRequest로 RefreshToken을 포함해서 요청했고, 그
                결과값으로 AccessToken을 반환받는 구조로 구현했었습니다.)
            Filter : 실제로 요청되지 전과 후에 URL 패턴에 맞는 요청을 처리하는 기능 제공
                - 세션 로그인 기준으로는 API 하나하나 마다 요청 전후에 검증이 필요했었습니다.(차후에)
