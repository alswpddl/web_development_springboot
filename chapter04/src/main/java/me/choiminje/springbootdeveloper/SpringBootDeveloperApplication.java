...
/*
    1. 테스트 코드 개념 익히기
        테스트 코드는 작성한 코드가 의도대로 잘 동작하고 예상치 못한 문제가 없는 지 확인할 목적으로 작성하ㅡㄴ 코드.
        보통 테스트 코드 관련 공부는 본 개발 공부를 하느라 미루는 경우가 많은데, 좀 익술해지면
            솔직히 이거 완전 좋습니다.
    ...
    이상의 코드를 확인하면 세 부분으로 나뉘어져 있음. 메뉴를 저장하기 위해 준비하느 과정은 given절
    실제로 메뉴를 저장하는 when절
    메뉴가 잘추가되었는지 확인하는 then절,.

    2. 스프링 부트 3와 테스트
        스트링 부트는 애플리케이션을 테스트하기 위한 도구와 애너테이션을 제공.
        spring-boots-starter-test 스타터에 테스트를 위한 도구가 모여있음.
