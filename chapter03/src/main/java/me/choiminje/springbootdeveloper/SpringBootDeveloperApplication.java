...
/*
    1. 스프링 부트 3 구조 살펴보기
        ...
        계층 : 각자의 역할과 책임이 있는 어떤 소프트웨어의 구성 요소
            -> 각 계층은 소통할 수는 있지만 다른 계층에 간섭하거나 영향을 미치진 않음
        ...
        스프링 부터에서의 계층
            1) 프레젠테이션(Presentation)
                HHTP 요청을 받고 이 요청을 비지니스 계층으로 전송하는 역할 -> Controller가
                    프레젠테이션 계층 역할
            ...
            3) 퍼시스턴스(Persistence)
                ...
                DAO(추후 수업 예정) 객체를 사용할 '수도' 있음.
                    * DAO - 데이터베이스 계층과 상호작용하기 위한 객체라고 이해하면 폅합니다 일단은.
        ...
        스프링 부트 프로젝트 디렉토리 구성
            : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따른다
                ...
                test : 프로젝트의 소스 코드를 테스트할 목적의 코드나 리소스 파일이 들어있음
                    (원래는 자동생성되는 어제 제가 만들었을 때 안만들어지는 걸로 보아 설정 따라 다른듯합니다)
                ...
                main 디렉토리 구성하기
                    ...
                    01 단계 - HTML과 같은 뷰 관련 파일(chapter07에서 수업 예정)을 넣은 templates 디렉토리
                    ...
                    03 단계 - 스프링 부트 설정을 할 수 있는 application.yml 파일을 생성
                        -> 해당 파일은 스프링 부트 서버가 실행되며 자동으로 로딩되는 파일로,
                        데이터베이스의 설정 저보, 로깅 설정 저보 등이 들어갈 수도 있고, 직접 정의할 때
                        사용하기도 함. 앞으로 프로젝트를 진행하며 자주 사용할 파일입니다.
                        resources 우클릭 -> new file -> applications.yml
        ...
        작동을 확인해보겠습니다.
            ...
            04 단계 - Postman에서 HTTP 요청을 시도해봅니다.
                ...
                2) HTTP 메서드를 Get으로 설정하고 URL에
                    http://localhos:8080/test로 설정(TestController.java 확인)
                3) SEND 버튼 누릅니다.
                4) 200 OK인지 확인

HTTP 요청 ----> TestController <----> TestService <----> MemberRepository <----> Database
url:/test----> 프로젠테이션 계층        비지니스 계층        퍼시스턴스 계층           데이터베이스
 */
