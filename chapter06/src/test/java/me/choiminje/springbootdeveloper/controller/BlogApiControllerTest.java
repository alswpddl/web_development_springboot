package me.choiminje.springbootdeveloper.controller;
...
import me.choiminje.springbootdeveloper.repository.BlogRepository;
...
//      import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;
        ...
        /*
            ...
            Article 객체의 field인 title이 "제목"인지, content가 "내용:인지 확인
    ...
    @DisplayName("findAllArticles : 볼로그 글 목록 조회 성공")
    @Test
    ...
    @DisplayName("findArticle : 볼로그 글 조회 성공")
    @Test
    ...
    /*
        삭제 구현하겠습니다.
            ...
            BlogServic로 가서 delete() 메서드 추가 -> blogRepository를 기준으로.
     */

    // 삭제 메서드
    @DisplayName("deleteArticle : 볼로그 글 삭제")
    @Test
    public void deleteArticle() throws Exception {
        ...
        final String title = "title";
        final String content = "content";
        // given - 2 객체 생성 -> 하나만 생성했다가 지운 다음에 List 전체 불러왔는데 0 이면 됩니다.
        Article savedArticle = blogRepository.save(Article.builder()
                .title(title)
                .content(content)
                .build());
        ...
        // then - 검증 단계에서 블로그 글 목록을 전부 다 가지고 올겁니다. -> size()체크 했더니 0이거나 / .isEmpty() 메서드 활용
        // 전부 다 가지고 오는 단계
