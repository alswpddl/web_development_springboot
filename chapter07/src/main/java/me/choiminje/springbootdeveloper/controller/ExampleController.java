...
public class ExampleController {
    ...
    public String thymeleafExample(Model model) { //import org.springframework.ui.Model;
        ...
        model.addAttribute("person",examplePerson);     // Person 객체를 "person" 뒤에 저장
        model.addAttribute("today", LocalDate.now());

        return "example";       // example.thml이라는 뷰를 조회합니다.
    }
...
/*
    ...
    thymeleaftExample()이라는 메서드는 "example"이라는 리턴값을 가지는데, 얘가 @Controller라는
...
컨트롤러(Controller)---------------------->           hobbies : ["운동", "독서"] -------------------> 뷰(View)
        ...
        다 하신 분들은 src/main/resources/templates 디텍토리에 example.html을 만들겁니다.
 */
