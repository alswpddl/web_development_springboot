...
-- 실무에서는 UNION ALL이 더 권자오딥니다. UNION의 경우 중복 제거가 있는데, 대량의 데이터를 대상으로
-- 중복 제거할 때 컴퓨터에 무리한 연산 부하를 줄 가능성이 있기 떄문에
...
-- 두 결과 집합을 UNION ALL을 사용해 하나로 결합하세요(단, 최종 결과는 최신순으로)
...
-- 컬럼 추출하고, 거주 국가 기준으로 알파벳 역순으로 출력하세요.)
...
--		UNION을 활용하여 orderdetails와 products를 FULL OUTER JOIN 조건으로 결합하여 
-- 		출력 하세요. -> 굳이 이런 형식으로 시험 문제 및 실무에까지 이용하는 경우는 거의 없습니다.

(select * from orderdetails o left join products p on o.product_id = p.id)
union
(select * from orderdetails o2 right join products p on o2.product_id = p.id)
...
-- orders와 staff를 활용해 last_name이 Kyle이나 Scott인 지원의 담당 주문을 출력하려면?
-- (딴, 서브쿼리 형태를 활용하자)

select id 
	from staff 
	where last_name int ("Kyle","Scott")				-- 조건절에 쓰일 경우에 scalar 서브가 아니었다는 점에 주목
...
-- WHERE 절 내에서 필터링 조건 지정을 위해 종첩된 서브 쿼리를 작성 가능
-- WHERE 에서 IN 연산자와 함께 서브 쿼리를 활용할 경우 :
--	컬럼 개수와 필터링 적용 대상 컬럼의 개수가 '일치'해아만 합니다.

-- 이상의 코드에서 서브쿼리의 결과 도출되는 컬럼의 숫자 = 1 (staff테이블의 id) / 행 = 2 (3, 5)
...
select *
	from (select * 
		from orders 
		where order_date >= "2015-07-01" 
			and order_date <= "2015-08-01") o		-- 1)의 결과가 테이블이었기 때문에 별칭 o를 표시
	inner join 
		(select * 
		from orderdetails 
...
-- 서브 쿼리를 작성하기 위한 바안 중에 하나는 서브 쿼리에 들어가게 될 쿼리문을 작성한 결과값을 확인.
...
-- 형태 :
-- ...
-- WHERE [컬렴명] [조건 연산자] ([서브 쿼리])
-- ...

-- WHERE에서 필터링을 위한 조건 값을 설정하는데 서브 쿼리르 사용 가능.
...
-- 5. 같은 대상 반복 대상 추출(LEFT JOIN)

-- users에서 created_at 컬럼 활용하여 연도별 가입 회원 수를 추출
