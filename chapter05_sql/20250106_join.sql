...
-- 1번 쿼리
select substr(created_at , 1, 7) as month, count(distinct id) as uniqueUser
	from users
	group by substr(created_at , 1, 7) as month
...
-- FROM에서 JOIN이 정렬된 후에 단일 테이블에 명령을 내리는 것처러 쿼리를 작성 가능
...
-- cf) 키 값은 테이블 내에서 고유한 값을 가지므로 한 테ㅣ블에서 개수를 계산할 때 중복되진 않는다.
-- 하지만 여러 테이블을 조인하면 '키 값도 중복될 수 있다.' 예를 들어 회원 아이디가 7인 사람이
-- 세 번 주문했다면 회원 정보(users)와 주문 정보(orders)를 결합한 결과에는 u.id = 7인
-- 행에 3 개 있을 것이다. 이 경우 '한 번 이라도 주문한 회원 수'를 중복 없이 구하려면 
-- 회원 id를 중복 제거한 뒤에 회원 수를 count할 필요가 있다.

-- key의 구분
-- 1. 기본 키(Primary Key) : 하나의 테이브렝서 가지는 고유한 값
...
-- 이를 JOIN시켰을 때 u.id로 ORDER BY하더라도 합친 테이블 상에서 제 1 컬럼(즉 PK 컬럼)에
...
-- 2.LEFT JOIN
-- user와 orders를 하나로 결합해 출력합니다(단, 주문 정보가 없는 회원의 정보도 모두 출력)
...
-- 이상의 예시에서는 users테이블이 왼쪽에 있으므로, users 테이블의 모든 결과값을 가져오는데,
-- orders 테이블에 대응하는 값이 없는 경우 null 값으로 출력됩니다.

-- 즉, 두 테이블의 교집합과 교집합에 속하지 않는 왼쪽 차집합을 불러오니다.
-- 왼쪽 테이블의 값을 전부 불러오기 때문에 LEFT JOIN / LEFT OUTER JOIN이라고 합니다. 

-- LEFT JOIN과 INNER JOIN은 함꼐 실무에서 자주 쓰입니다. 데이터를 결합하는 경우 대부분
...
-- '결합 후에' 컬럼 값에 접근할 때는 [테이브 별칭].[컬럼명]으로 내부 컬럼에 접근합니다.
...
-- 첫 번쨰 JOIN의 ON절 뒤에, 두 번째 JOIN 절을 작성하면 됩니다.
	
select u.username, u.phone, o.user_id, o.id, od.order_id, od.product_id
...
-- u.id 와 o.user_id를 연결하는 등의 조건 없이 두 테이블의 모든 해을 합쳐서 만들 수 있는 모든
-- 경우의 수를 생성한 것입니다.
...
-- 연습 문제
-- 1. users와 staff를 참고하여 회원 중 직원인 사람이 회원 id, 이메일, 거주 도시, 거주 국가,
-- 성, 이름을 한 화면에 출력하세요.
...
select o.user_id
		, sum(p.price * o2.quantity) as sumPrice
		, sum(p.discount_price * o2.quantity) as sumDiscountPrice
...
-- 또한 두 테이블 사이에 고통된 컬럼 값인 키 값이 존재해야만 JOIN으로 결합할 수 있습니다.
