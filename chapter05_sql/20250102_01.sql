...
-- 거주 구가가 한국이 아닌 회원만 추출하는 쿼리문을 작성하시오.
select * from users where country != "Korea";
...
-- where절을 작성하는데 있어서 컬럼명이 먼저 나오고 =, !=, betweenemddmf wjrdyd
...
-- LIKE : 해당 전치사 뒤에 작은따옴표/큰따옴표 내에서는 와일드카드를 사용 가능
-- SQL을 해석하는 컴퓨터가 LIKE 코드를 읽는 순간 와일드카드를 감지하는데
-- SQL에서의 와일드 카드는 '%'로 0개 이상의 임의의 문자열

-- 예제 users에서 country의 
-- 거주 국가가 S로 시작하는 국가(ex)South Korea)정보 모두 출력
-- 거주 국가가 S로 끝나는 국가 정보 모두 출력
...
select created_at, phone, city, country
	...
	-- 2. products에서 id가 20 이하이고 price는 30 이상인 제품 정보 출력
		...
		-- discount_amount라는 컬럼명으로 추출할 것.
							-- '전부 다'의 기준은 기존 컬럼
select *, (price - discount_price) as discount_amount	-- 새로운 컬럼
	from products
...
select id, name, price
	from products
	where name like "N%";
	-- 5. orders에서 order_data가 2015-07-01부터 2015-10-31까지가 
		-- 아닌 정보만 출력할 것.
select *
	from orders
	where order_date not between "2015-07-01" and "2015-10-13"
...
select *
	from users u 
	order by id asc;		-- ASC : Ascending의 축약어 '오름차순'
...
-- 에제 : user에서 city를 기준으로 내림차순 하여 전체 정보 출력해보세요.
select *
	from users u 
	order by city desc;
...
-- 예제 : user에서 첫 번째 컬럼 기준으로 오름차순 정렬하여 출력
select *
	from users u 
	order by 1 asc;
...
select username, phone, city, country, id 
	from users
	order by 1 asc;
-- 아까와 같이 ORDER BY 1 ASC로 했지만 정렬이 id가 아닌 usernam을 기준으로 
-- ASCII Code로 적용되어 정렬 방식이 달라졌음을 확인 가능.
...
select *
	from 음식점 정보
	where 지역 = "마포구" and 1인분가능여부 = 1;		-- 1을 쓸 경우에는 True의미 / 0은 False 의미
