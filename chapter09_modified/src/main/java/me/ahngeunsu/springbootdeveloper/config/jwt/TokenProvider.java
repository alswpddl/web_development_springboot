...
/*
    1. makeToken():
        ...
        header에는 type, 내용에 iss, iat, exp, sub
        claim에는 유저 id 지정
        토큰을 만들 때 properties 파일에 선언해둔 비밀 값과 함께 HS256 방식으로 암호화

    2. validToken():
        토큰이 유효한지 검증하는 메서드. 프로퍼티즈 파일에 선언한 비밀 값과 함께 톸느 복호화
    ...
    4. getUserId():
        토큰 기반으로 사용자 ID를 가져오는 메섣. 프로퍼티즈 파일에 저장한 비밀값으로
        토큰을 복호화한 다음 클레임이 가져오는 private getClaims()를 호출,
        클레임 정보를 반환받고, 클레임 id 키로 저장된 값을 가져와 반환.

        다음 단계 -> 코드가 제대로 동작하는 확인하기 위해서 테스트 동작 코드 작성 예정
            20250116에 하겠습니다.
