...
/*
    1. Spring vs Springboot
        ...
        Enterprise Apllication : 대규모의 복잡한 데이터를 관리하는 애플리케이션
            ...
            모든 걸 신경쓰면서 사이트 가능, 즉 '비지니스 로직'을 개발하기가 매우 어려워서
            등장하게 된 것이 Spring
        ...
        2. 스프링 컨셉
            1) 제어의 역전 (IoC)
                Inversion of Control : 다른 객체를 직접 생성하거나 제어하는 것이 아니라
                    ...
                    ex) 클래스 A에서 클래스의 B의 객체를 생성한다고 가정했을 때의 자바 코드
                    public class A {
                        B b = new B();
                    }
            ...
            3) 빈과 스프링 컨테이너
                ...
                3)-2. 빈 : 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                    ...
                    public class MyBean{}
                        ...
                        이상과 같이 @component라는 애너테이션을 붙이면 MyBean 클래스가 빈으로 등록함
            ...
            4) 관점 지향 프로그래밍(AOP)
                Aspect-oriented Programming : 프로그래밍에 대한 관심을 핵심 관점 / 부가 관점
                    ...
                    로깅 조작, 즉 지금까지 일어난 일을 기록하기 위한 로직과 여러 데이터를 관리
    ...
    3. 스프링 부트 3 둘러보기
        ...
        스프링 부트 스타터 살펴보기
            ...
            spring-boot-starter-actuator : 모니터링을 위해 애프리케이션에서 제공하는 다양한 정보를
                제공하기 쉽게 하는 의존성 모음
