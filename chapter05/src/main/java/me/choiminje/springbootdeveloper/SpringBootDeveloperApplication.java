...
/*
    5장. '데이터베이스' 조작이 편해지는 'ORM'
        1. 데이터베이스(database)란?
            ...
            DBMS(DataBase Management System) : 기본적으로 데ㅣ터베이스는 많은 사람이 공유할 수 있어야 하고,
                동시접근이 가능해야 하는 등 많은 요구 사항이 존재함.
                ...
                관계형 DBMS
                    ...
                    RDMBS는 테이블('표'를 의미) 형태로 이루어진 데이터 저장소로, 예를 들어
                    이하와 같은 회워 테이블이 있다고 가정할 때,
                   ...
                   +----------------------------------+
                    ...
                    H2, MySQL
                        ...
                        H2 - 다바로 작성되어 있는 RDBMS : 스프링 부트가 지원하는 인메모리 관계형 데이터베이스
                    ...
                    필수 용어
                        1) 테이블 : DB에서 데이터를 구성하기 위한 가장 기본적인 단위 행과 열로 구성
                        ...
                        3) 열(column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                            예를 들어 회원 테이블이 있다고 할 떄, 열은 각 요소에 대한 속성을 나타내며(Java에서는 field),
                            무결성을 보장함. 이상의 표를 기준으로 할 떄는, 이메일은 문자열일 것, 나이는 숫자 유형을 가짐.
                        ...
                        4) 기본키(Primary Key) : 행을 구분할 수 있는 식별자. 이 값은 테이블에서 유일해야 하며 '중복값을 가질 수 없음'
                            ...
                            또한 기본키의 값을 수정되어서는 안되며 유효한 값이어야 함. 즉, NULL이 될 수 없음
                                -> 이전 수업에서 nullable = false
                        5) 쿼리(query) : 데이터베이스에서 데이터를 조회하거나 삭제, 생성, 수정과 같은 처리를 하기 위해 사용되는 명령문
        ...
        2. ORM이란?
            Object-Relational Mapping이라고 하며 자바의 객체와 데이터베이스를 연결하는 프로그래밍 기법
                예를 들어 데이터베이스에 age, name 컬럼에 20, 홍길동이라는 값이 있다고 가정했을 떄, 이것을 자바에서 사용하려면
                SQL을 이용하여 데이터를 꺼내 사용하지만, ORM이  있다면 데이터베이스의 값을 마치 객체처럼 사용할 수 있음.
                즉, SQL에 어령무을 겪는다고 하더라도 자바 언어로만 데이터베이스에 접근해서 원하는 데이터를 받아올 수 있는 방식.
